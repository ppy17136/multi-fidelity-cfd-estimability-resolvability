...
def main():
    ...
    # The frozen record identifies the original implementation, not the
    # corrected runtime module. Verify that historical artifact explicitly;
    # every scientific output below is still recomputed with the current code.
    historical=PACKAGE/'docs/lineage/graph_constrained_coupling_v2_1_8.py.txt'
    assert hashlib.sha256(historical.read_bytes()).hexdigest()==expected['graph_module_sha256']
    current_graph_sha256=summary['graph_module_sha256']
    for key,value in summary.items():
        if key=='graph_module_sha256':
            continue  # historical identity checked above, not a numerical output
    ...
    print('Historical graph module verified:',expected['graph_module_sha256'])
    print('Current recomputation graph module:',current_graph_sha256)
