# Frozen method-upgrade protocol
...
The existing V8 reproducibility package remains immutable and is not overwritten.
...
1. **Minimum-cost support repair** 鈥?exact and greedy solvers verified against
   the frozen benchmark suite.
2. **Contrast-level numerical evidence propagation** 鈥?cell-to-contrast
   propagation implemented and its evidential scope stated.
3. **State-dependent acquisition** 鈥?proposed policy compared under identical
   candidate pools and costs.
4. **Closed-loop CFD evidence repair** 鈥?at least one prospective repair path
