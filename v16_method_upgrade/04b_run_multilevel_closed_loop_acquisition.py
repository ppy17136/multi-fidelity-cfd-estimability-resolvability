...
def choose_action(
    ...
    if policy in {"c_optimal", "support_then_c_optimal"}:
        if not estimable(mapping):
            plan = support_plan(mapping, available)
            return min(plan, key=lambda action: (action.cost, action.name)) if plan else None
...
def draw_estimate(
    ...
    return float(
        truth
        + rng.uniform(-mapping_bound, mapping_bound)
        + rng.uniform(-repeatability_bound, repeatability_bound)
    )
...
def apply_action(
    ...
    if action.kind == "row":
        ...
        estimates[cell] = draw_estimate(
            truth[cell], mapping[cell], repeatability[cell], rng
        )
    elif action.kind == "mapping":
        cell = action.cell
...
def run_one(
    policy: str, true_delta: float, rng: np.random.Generator
) -> dict:
    ...
    for cell, q, r in ((0, 0.45, 0.10), (2, 0.30, 0.08)):
        ...
        estimates[cell] = draw_estimate(truth[cell], q, r, rng)
    ...
    while spent < BUDGET and state == "indeterminate":
        ...
        estimates, mapping, repeatability = apply_action(
            action, truth, estimates, mapping, repeatability, rng
        )
        spent += action.cost
    ...
    truth_state = (
        "effect_present"
        if abs(true_delta) > MINIMUM_EFFECT
        else "effect_absent"
    )
    return {
        "policy": policy,
...
def main() -> None:
    ...
    children = seed_sequence.spawn(len(policies) * len(true_deltas) * REPLICATES)
    rows = []
    child = 0
    for true_delta in true_deltas:
        for policy in policies:
            for replicate in range(REPLICATES):
                ...
                child += 1
                row = run_one(policy, true_delta, rng)
                row["replicate"] = replicate
                rows.append(row)

    with (ROOT / "04b_multilevel_closed_loop_results.csv").open(
    ...
    for true_delta in true_deltas:
        for policy in policies:
            ...
            valid_costs = np.asarray(
                [row["cost"] for row in subset if row["valid_decision"]],
                dtype=float,
            )
            paths, counts = np.unique(
                [row["actions"] for row in subset], return_counts=True
            )
            summary_rows.append(
                {
                    "true_delta": true_delta,
                    ...
                    "valid_decision_fraction": float(
                        np.mean([row["valid_decision"] for row in subset])
                    ),
                    "false_present_fraction": float(
                        np.mean([row["false_present"] for row in subset])
                    ),
                    "false_absent_fraction": float(
                        np.mean([row["false_absent"] for row in subset])
                    ),
                    "indeterminate_fraction": float(
                        np.mean([row["indeterminate"] for row in subset])
                    ),
                    "median_cost_to_valid_decision": (
                        float(np.median(valid_costs)) if len(valid_costs) else None
                    ),
                    "median_cost_bootstrap_95": (
                        bootstrap_median_ci(valid_costs, bootstrap_rng)
                        if len(valid_costs)
                        else None
                    ),
    ...
    summary = {
        ...
        "rows": summary_rows,
        "checks": {
            "all_false_present_zero": all(
                row["false_present_fraction"] == 0 for row in summary_rows
            ),
            "all_false_absent_zero": all(
                row["false_absent_fraction"] == 0 for row in summary_rows
            ),
        },
        "claim_boundary": (
            "Constructed verification of acquisition logic under declared "
            "bounded errors; not external CFD validation."
        ),
    }
